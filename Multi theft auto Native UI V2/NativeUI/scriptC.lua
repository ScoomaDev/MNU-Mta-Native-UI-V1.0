   ...
   -- dxDrawRectangle(panelX, panelY, panelWidth, notificationData["height"], tocolor(0, 0, 0, 210))
    ...
    for k = window.startRow, window.startRow + window.iVisibleRow - 1, 1 do
        ...
        if (k ~= currentUse) then
            ...
            if v[6] then
                ...
                dxDrawText(v[6], panelX+20*relX, currentY+(rowHeight*(window.iVisibleRow +0.3 )), panelWidth+panelX+10*relX, rowHeight+currentY+(rowHeight*(window.iVisibleRow +0.3 )), tocolor(255,255,255,255), 0.4, chaletlondon, "left", "center",false,true)
            end
        ...
        dxDrawText(v[2], panelX+10*relX, currentY+(rowHeight*(k-1-window.startRow)), panelWidth+panelX+10*relX, rowHeight+currentY+(rowHeight*(k-1 - window.startRow)), textColor, 0.45, chaletlondon, "left", "center")
        local state = "□"
        if (v[1] == "check") then
        ...
        dxDrawText(state, panelX+10*relX, currentY+(rowHeight*(k-1- window.startRow)), panelWidth-20*relX+panelX+10*relX, rowHeight+currentY+(rowHeight*(k-1- window.startRow)), textColor, 1.0, chaletlondon, "right", "center")
    ...
    elseif (v[1] == "select") then 
        ...
        if currentUseText then 
            dxDrawText("< "..currentUseText.." >", panelX+10*relX, currentY+(rowHeight*(k-1- window.startRow)), panelWidth-20*relX+panelX+10*relX, rowHeight+currentY+(rowHeight*(k-1- window.startRow)), textColor, 0.55, chaletlondon, "right", "center")
        end
    end
end
end
